...
SCRIPT_VERSION = '1.0.4'  # Update with each release.
...
def display_as_table(tickets):
    ...
    field_names = ["id", "department_id", "subject", "priority", "status", "is_escalated", "environment", "account_tier", "ticket_type", "created_at"]
    
    # Check if debug mode is on and append 'Score' to field names
    if logging.getLogger().getEffectiveLevel() == logging.DEBUG:
    ...
    for ticket in tickets:
        ...
        # Default account_tier to 'C' if it's None or empty
        account_tier = ticket['custom_fields'].get('account_tier') or 'C'
        
        # Default environment to 'Production' if it's None or empty
        environment = ticket['custom_fields'].get('environment') or 'Production'

        # Prepare the basic row
        row = [
            ticket['id'],
            ...
            ticket['status'],
            ticket['is_escalated'],
            environment, # Use the environment variable here
            account_tier,  # Use the account_tier variable here
            ticket['custom_fields']['ticket_type'],
            created_at_local,
        ]
        
        # Append score if debug mode is on
        if logging.getLogger().getEffectiveLevel() == logging.DEBUG:
